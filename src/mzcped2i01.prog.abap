  ...
  CASE vg_ok_code.
    ...
    WHEN 'NEXT'.
      IF vg_scaner IS NOT INITIAL.
        ...
        ELSE.
          PERFORM f_arma_102 USING 'UnManipu' vg_scaner 'Ya fue asignado' space space space
  ...
  CASE vg_ok_code.
    WHEN 'SAVE'.
      IF tg_dele[] IS NOT INITIAL.
        DELETE zsdtt_002 FROM TABLE tg_dele.
        CLEAR tg_dele[].
        ...
        IF sy-subrc EQ 0.
          LOOP AT tg_p102 INTO sg_p102
            ...
            LOOP AT tg_p101 INTO sg_p101
              ...
              sg_p101-scaneo = sg_p101-scaneo - 1.
              sg_p101-lfimg  = sg_p101-lfimg - sg_p102-lfimg.
              sg_p101-pza    = sg_p101-lfimg / 100.
              MODIFY tg_p101 FROM sg_p101.
            ENDLOOP.
            IF sy-subrc = 0.
              APPEND sg_p102 TO tg_dele.
            ENDIF.
            DELETE tg_p102.
          ENDLOOP.
          IF sy-subrc = 0.
            DELETE tg_p101 WHERE lfimg EQ 0.
          ELSE.
            " Mensaje de que no exite
          ENDIF.
*            DELETE tg_p102 WHERE exidv EQ vg_scaner.
*        ENDIF.
        ELSE.
...
MODULE user_commands_0100 INPUT.
  ...
  IF vg_nocarga IS NOT INITIAL.
    ...
    PERFORM f_lee_datos USING sy-datum.

    IF sy-subrc EQ 0.

      CLEAR tg_p101[].
      PERFORM f_arma_102 USING 'No de carga' vg_nocarga 'ya fue asignado' space space space
                               space space space space.
...
MODULE user_commands_0101 INPUT.
  ...
  IF vg_nocarga IS NOT INITIAL.
    ...
    ELSE.
      ...
      CLEAR sg_p101.
      LOOP AT tg_p102 INTO sg_p102.
        PERFORM f_arma_visual USING sg_p102-matnr sg_p102-lfimg 1.
      ENDLOOP.
      vg_fecha = sg_p102-fecha.
      IF vg_agrega EQ abap_true.
        CALL SCREEN '0102'.
      ENDIF.
      IF vg_quitar EQ abap_true.
        CALL SCREEN '0104'.
      ENDIF.
