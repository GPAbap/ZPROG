...
*  IF s_bstyp-low EQ 'A'.                "vmr02- Incidente 240
  lr_bukrs[] = gr_bukrs[].
  "vmr02-INI Incidente 240
*  ELSEIF s_bstyp-low EQ 'F'.
*    CLEAR lr_bukrs.
*  ENDIF.
  "vmr02-FIN Incidente 240
...
*vmr<--FIN
    SELECT * FROM ekko INNER JOIN ekpo ON ( ekko~ebeln = ekpo~ebeln )
       APPENDING CORRESPONDING FIELDS OF TABLE gt_alv
        WHERE ekko~ebeln IN s_ebeln
          ...
          AND ekko~reswk IN s_reswk
          AND ekko~bedat IN s_bedat
          AND ekko~bsart IN gr_bsart
          AND ekko~frgke IN gr_frgke
          AND ekpo~matnr IN s_matnr
          AND ekpo~matkl IN s_matkl
          AND ekpo~loekz EQ ' '.
  ENDIF.    "vmr
  ...
  IF gt_alv[] IS NOT INITIAL.
    LOOP AT gt_alv ASSIGNING <fs_alv>.
      ...
      SELECT SINGLE name1 adrnr FROM lfa1 INTO (<fs_alv>-name1, <fs_alv>-adrnr)
        WHERE lifnr EQ <fs_alv>-lifnr.
      IF sy-subrc EQ 0.
        SELECT SINGLE smtp_addr FROM adr6 INTO <fs_alv>-smtp_addr
      ...
      ENDIF.
*vmr-->
*      IF <fs_alv>-bsart <> 'ZPRE' AND <fs_alv>-frgke NOT IN gr_frgke.
*        DELETE gt_alv.
*      ENDIF.
*vmr<--
    ENDLOOP.
...
*       HEADER                  =
    ...
    ELSE.
      ...
      IF sy-subrc EQ 0.
        lv_titulo = wa-line. " Se asigna Titulo
...
*&---------------------------------------------------------------------*
FORM f_fill_ranges .
  CLEAR: gr_bsart[], gr_bsartn[], gr_frgke[], gr_frgken[].

*vmr01--> Tipo de documentos a considerar no liberados
  SELECT * FROM tvarvc
    INTO TABLE @DATA(lt_low1)
    WHERE name = 'ZMM_NO_LIB_CB'.
  IF sy-subrc EQ 0.
