...
MODULE exit_commands_0100 INPUT.
  ...
  LEAVE PROGRAM.
  CLEAR: vg_index,vg_line,vg_lines,vg_pallet,vg_pos.
...
MODULE user_commands_0100 INPUT.
  ...
  CASE vg_ok_code.
    WHEN 'NEXT'.
      ...
      " Buscamos los datos de cabecera
      SELECT SINGLE handle huinv_nr werks lgort inventory counted posted
        INTO sg_head
        FROM huinv_hdr
      ...
      IF sy-subrc = 0.
        ...
        AND sg_head-counted EQ abap_true.
          PERFORM f_arma_102 USING '' 'DocInv:' vg_huinv_nr 'Compensado' 'con conteo previo'
...
MODULE user_commands_0101 INPUT.
  ...
  CASE vg_ok_code.
    ...
    WHEN 'NEXT'.
      ...
      AND vg_top_exidv NE vg_pallet.
        vg_pos = vg_pos + 1.
        LOOP AT tg_item INTO sg_item WHERE exidv = vg_top_exidv.
          sg_item-mark    = abap_true.
          sg_item-huexist = abap_true.
          sg_item-index   = vg_pos.
      ...
      IF sy-subrc NE 0.
        ...
        ENDIF.
        SORT tg_item BY index DESCENDING item_nr venum top_exidv.
      ENDIF.
      CLEAR vg_top_exidv.
...
MODULE exit_commands_0101 INPUT.
  ...
  CLEAR: tg_item[].
  CLEAR: vg_index,vg_line,vg_lines,vg_top_exidv,vg_pallet,vg_pos.
